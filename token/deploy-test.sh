...
printf "Code id is %s\n" $code
addr=$(printf $pass | $seid tx wasm instantiate $code '{}' --from=wallet --broadcast-mode=block --label "EGG" --chain-id=atlantic-2 --gas=300000 --fees=30000usei --admin=sei18dl724gejf2l6eza9x5gg00s4nx4hkqs5dkva4 -y --node=https://sei-testnet-rpc.polkachu.com | grep -A 1 -m 1 "key: _contract_address" | sed -n 's/.*value: //p' | xargs)
