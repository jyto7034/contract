...
printf "Code id is %s\n" $code
addr=$(printf $pass | $seid tx wasm instantiate $code '{}' --from=mainnet-wallet --broadcast-mode=block --label "Token Factory" --chain-id=pacific-1 --gas=300000 --fees=30000usei --admin=sei17k3ah8kd2k6u4prlsvm8f3psghw2at7cf2mjpr -y --node=https://sei-rpc.polkachu.com | grep -A 1 -m 1 "key: _contract_address" | sed -n 's/.*value: //p' | xargs)
